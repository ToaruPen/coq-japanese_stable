...
5. Verify using `just` recipes where available.
   ...
   - If a CodeRabbit fix changes `ColorAwareTranslationComposer`, `Strip`, or adds a cataloged color-preserving translation call such as `ColorAwareTranslationComposer.TranslatePreservingColors(...)` or `GetDisplayNameRouteTranslator.TranslatePreservingColors(...)`, run the color-route catalog and strip/restore allowlist tests before the broad gate. Those deterministic inventories are part of the color-preservation contract, not incidental snapshots.
