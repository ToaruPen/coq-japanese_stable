...
[HarmonyPatch]
public static class RegenerationTranslationPatch
{
    private const string Context = nameof(RegenerationTranslationPatch);
    ...
    private static IEnumerable<MethodBase> TargetMethods()
    {
        ...
        var targetType = AccessTools.TypeByName("XRL.World.Parts.Mutation.Regeneration");
    ...
    internal static bool TryTranslateQueuedMessage(ref string message, string? color)
    {
        ...
        if (!string.Equals(message, "{{G|You were decapitated, but a new head regrew immediately!}}", StringComparison.Ordinal))
        {
            return false;
        }

        const string translated = "{{G|首を刎ねられたが、すぐに新しい頭が生えた！}}";
