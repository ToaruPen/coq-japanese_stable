...
[HarmonyPatch]
public static class StatusScreenTranslationPatch
{
    private const string Context = nameof(StatusScreenTranslationPatch);

    private static readonly HashSet<string> TranslatableLiterals = new(StringComparer.Ordinal)
    {
        "< {{W|7}} Skills | Inventory {{W|9}} >",
        " Skills | Inventory ",
    };

    private static readonly MethodInfo TranslateLiteralMethod =
        AccessTools.Method(typeof(StatusScreenTranslationPatch), nameof(TranslateLiteral))
    ...
    private static readonly MethodInfo TranslateRenderedPromptMethod =
        AccessTools.Method(typeof(StatusScreenTranslationPatch), nameof(TranslateRenderedPrompt))
        ?? throw new InvalidOperationException("TranslateRenderedPrompt method not found.");

    [HarmonyTargetMethod]
    private static MethodBase? TargetMethod()
    {
        var targetType = GameTypeResolver.FindType("XRL.UI.StatusScreen", "StatusScreen");
    ...
    public static string TranslateLiteral(string source)
    {
        try
        {
            return LegacyGamepadPromptTranslationHelpers.TranslateStatusLiteral(source);
        }
        catch (Exception ex)
        {
    ...
    public static string TranslateRenderedPrompt(string source)
    {
        try
        {
            return LegacyGamepadPromptTranslationHelpers.TranslateStatusRendered(source);
        }
        catch (Exception ex)
        {
