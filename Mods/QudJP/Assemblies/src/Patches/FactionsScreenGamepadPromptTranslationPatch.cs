...
[HarmonyPatch]
public static class FactionsScreenGamepadPromptTranslationPatch
{
    private const string Context = nameof(FactionsScreenGamepadPromptTranslationPatch);

    private static readonly HashSet<string> TranslatableLiterals = new(StringComparer.Ordinal)
    {
        "< {{W|7}} Equipment | Quests {{W|9}} >",
        " Equipment | Quests ",
    };

    private static readonly MethodInfo TranslateLiteralMethod =
        AccessTools.Method(typeof(FactionsScreenGamepadPromptTranslationPatch), nameof(TranslateLiteral))
    ...
    private static readonly MethodInfo TranslateRenderedPromptMethod =
        AccessTools.Method(typeof(FactionsScreenGamepadPromptTranslationPatch), nameof(TranslateRenderedPrompt))
        ?? throw new InvalidOperationException("TranslateRenderedPrompt method not found.");

    [HarmonyTargetMethod]
    private static MethodBase? TargetMethod()
    {
        var targetType = GameTypeResolver.FindType("XRL.UI.FactionsScreen", "FactionsScreen");
    ...
    public static string TranslateLiteral(string source)
    {
        try
        {
            return LegacyGamepadPromptTranslationHelpers.TranslateFactionsLiteral(source);
        }
        catch (Exception ex)
        {
    ...
    public static string TranslateRenderedPrompt(string source)
    {
        try
        {
            return LegacyGamepadPromptTranslationHelpers.TranslateFactionsRendered(source);
        }
        catch (Exception ex)
        {
