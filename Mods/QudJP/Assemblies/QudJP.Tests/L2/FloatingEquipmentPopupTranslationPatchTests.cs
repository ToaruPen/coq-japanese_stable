...
public sealed class FloatingEquipmentPopupTranslationPatchTests
{
    ...
    [TestCase(nameof(DummyFloatingEquipmentProducer.MagnetizedCheckFloating), "The {{Y|floating orb}} falls to the ground; you scoop it up.", "{{Y|floating orb}}は地面に落ちた。あなたはそれをすくい上げた。", "FloatingEquipmentFall")]
    public void Patch_TranslatesFloatingEquipmentPopup_MagnetizedWithScoopAction_WhenOwnerPatched(
        string methodName,
        string source,
        string expected,
        string expectedFamilySuffix)
    ...
    {
        const string source = "The {{Y|hover boots}} fly off into the distance.";
        ...
        RunWithOwnerAndPopupPatches(nameof(DummyFloatingEquipmentProducer.PoweredCheckFloating), () =>
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(GetFallHitCount(), Is.Zero);
            });
        });
    }

    [Test]
    public void Patch_HitCountsRemainZero_WhenEmptyPopupEncountered_WhenOwnerPatched()
    {
        RunWithOwnerAndPopupPatches(nameof(DummyFloatingEquipmentProducer.PoweredCheckFloating), () =>
        {
            var target = new DummyFloatingEquipmentProducer
