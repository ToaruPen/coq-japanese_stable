...
public sealed class DeployableInfrastructurePopupTranslationPatchTests
{
    ...
    [Test]
    public void AttemptDeploy_NoUsefulWayPopup_LeavesPopupUnchanged_WhenOwnerAbsent()
    {
        const string source = "There is no useful way to deploy {{Y|ワイヤー}} there.";
        var target = new DummyDeployableInfrastructureTarget
        {
            PopupMessageToSend = source,
            UseShowFail = true,
    ...
    [Test]
    public void AttemptDeploy_HitCountRemainsZero_ForUnknownPopup_WhenOwnerPatched()
    {
        var target = new DummyDeployableInfrastructureTarget
        {
            PopupMessageToSend = "The device malfunctions.",
        };
        ...
        Assert.Multiple(() =>
        {
            Assert.That(DummyPopupShow.LastShowMessage, Is.EqualTo("The device malfunctions."));
            Assert.That(DeployableInfrastructureHitCount("DoesVerb"), Is.Zero);
            Assert.That(DeployableInfrastructureHitCount("NoUsefulWay"), Is.Zero);
        });
    }
