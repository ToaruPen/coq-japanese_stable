...
public sealed class CombatAndLogMessageQueuePatchTests
{
    ...
    [TestCase("You don't penetrate Snapjaw Scavenger's armor.", "Snapjaw Scavengerの防具を貫通できなかった！")]
    [TestCase("You don't penetrate Snapjaw Scavenger's armor with your bronze dagger. [17]", "bronze daggerではSnapjaw Scavengerの装甲を貫けない。[17]")]
    [TestCase("You don't penetrate Snapjaw Scavenger's armor. [17]", "Snapjaw Scavengerの装甲を貫けない。[17]")]
    [TestCase("Snapjaw Scavenger doesn't penetrate your armor.", "Snapjaw Scavengerはあなたの防具を貫通できなかった")]
    [TestCase("Snapjaw Scavenger doesn't penetrate your armor with its bronze dagger! [17]", "Snapjaw Scavengerはbronze daggerであなたの防具を貫通できなかった！ [17]")]
    [TestCase("Snapjaw Scavenger doesn't penetrate your armor! [17]", "Snapjaw Scavengerはあなたの防具を貫通できなかった！ [17]")]
    ...
    [Test]
    public void CombatTextSurface_DoesNotRetranslateDirectMarkedQueuedMessage_InShieldOwnerScope()
    {
        UseRepositoryPatternDictionary();
        ...
        try
        {
            ...
            var markedMessage = MessageFrameTranslator.MarkDirectTranslation("You block with iron buckler! (+2 AV)");
            var target = new DummyCombatGetDefenderHitDiceTarget
            {
                MessageToSend = markedMessage,
            };
    ...
    [Test]
    public void CombatGetDefenderHitDice_DoesNotTranslateUnmatchedMessage_InShieldOwnerScope()
    {
        WritePatternDictionary(
            ("^You block with (.+)! \\(\\+(\\d+) AV\\)$", "{0}で防御した！ (+{1} AV)"));
        ...
        try
        {
            ...
            var target = new DummyCombatGetDefenderHitDiceTarget
            {
                MessageToSend = "You deal 5 damage to Snapjaw Scavenger.",
            };

            target.HandleEvent(new DummyCombatGetDefenderHitDiceEvent());

            Assert.That(DummyMessageQueue.LastMessage, Is.EqualTo("You deal 5 damage to Snapjaw Scavenger."));
        }
        finally
        {
            harmony.UnpatchAll(harmonyId);
    ...
    [Test]
    public void CombatMeleeAttack_DoesNotTranslateUnmatchedMessage_InMeleeOwnerScope()
    {
        WritePatternDictionary(
            ("^You miss! \\[(.+?) vs (.+?)\\]$", "攻撃は外れた！ [{0} vs {1}]"));
    ...
    [Test]
    public void CombatMeleeAttack_TranslatesColorTaggedPenetrateMessage_WhenPatched()
    {
        WritePatternDictionary(
            ("^You don't penetrate (?:the )?(.+?)(?:'s|s') armor\\.$", "{0}の防具を貫通できなかった！"));
        ...
        try
        {
            ...
            var target = new DummyCombatMeleeAttackTarget
            {
                MessageToSend = "{{r|You don't penetrate}} Snapjaw Scavenger's armor.",
                ColorToSend = null,
            };

            _ = target.MeleeAttackWithWeaponInternal(new DummyGameObject(), new DummyGameObject(), new DummyGameObject(), new DummyCombatBodyPart());

            Assert.That(DummyMessageQueue.LastMessage, Is.EqualTo("{{r|Snapjaw Scavengerの防具を貫通できなかった！}}"));
        }
        finally
        {
            harmony.UnpatchAll(harmonyId);
    ...
    [TestCase("You don't penetrate Snapjaw Scavenger's armor.", "Snapjaw Scavengerの防具を貫通できなかった！")]
    [TestCase("You don't penetrate the iron golem's armor.", "iron golemの防具を貫通できなかった！")]
    [TestCase("You don't penetrate Snapjaw Scavengers' armor.", "Snapjaw Scavengersの防具を貫通できなかった！")]
    public void CombatMeleeAttack_TranslatesNoPenetrateWithoutRoll_WithRepositoryPatterns(string source, string expected)
    {
        AssertCombatMeleeAttackQueuedMessageWithRepositoryPatterns(source, expected);
    }
