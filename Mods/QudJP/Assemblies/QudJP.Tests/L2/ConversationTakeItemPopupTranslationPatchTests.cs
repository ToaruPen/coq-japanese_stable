...
public sealed class ConversationTakeItemPopupTranslationPatchTests
{
    ...
    [Test]
    public void Execute_TranslatesTakeSuccessPopup_WithDifferentActorName_WhenOwnerPatched()
    {
        AssertPopupMessage(
            "Sheba Hagadias takes {{Y|ancient tablet}}.",
            "Sheba Hagadiasは{{Y|ancient tablet}}を受け取った。");

        Assert.That(TakeItemHitCount(), Is.EqualTo(1));
    }

    [Test]
    public void Execute_TranslatesCannotGivePopup_WithUntaggedItemName_WhenOwnerPatched()
    {
        DummyConversationTakeItemTarget.PopupMethod = nameof(DummyPopupShow.ShowFail);

        AssertPopupMessage(
            "You cannot give item!",
            "itemを渡せない！");
    ...
    [Test]
    public void Execute_HitCountRemainsZero_WhenUnknownPopupEncountered_WhenOwnerPatched()
    {
        const string source = "Q Girl inspects {{Y|奇妙な小物}}.";
