...
public sealed class GritGateTerminalKnowledgePopupTranslationPatchTests
{
    ...
    [Test]
    public void Activate_TranslatesInsightPopup_PreservesBodyTextUntranslated_WhenOwnerPatched()
    {
        const string bodyText = "Bethesda Susa can be found in the northeast region of the world map. The facility " +
            "is defended by Barathrumite guards.";
        var source = "Ereshkigal delivers insight from the Thin World:\n\n" + bodyText;
        var expected = "エレシュキガルは薄界からの洞察を授ける:\n\n" + bodyText;
    ...
    [Test]
    public void Activate_TranslatesInsightPopup_WhenBodyContainsColorTags_WhenOwnerPatched()
    {
        const string source = "Ereshkigal delivers insight from the Thin World:\n\nThe {{Y|Legendary}} creature {{R|Haggabah}} can be found at {{G|Golgotha}}.";
        const string expected = "エレシュキガルは薄界からの洞察を授ける:\n\nThe {{Y|Legendary}} creature {{R|Haggabah}} can be found at {{G|Golgotha}}.";
    ...
    [Test]
    public void Activate_LeavesHitCountAtZero_WhenEmptyPopupEncountered_WhenOwnerPatched()
    {
        AssertPopupMessage(string.Empty, string.Empty);
