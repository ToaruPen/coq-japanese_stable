...
public sealed class TinkeringBuildPopupTranslationPatchTests
{
    ...
    [TestCase("You tinker up one {{Y|freeze grenade}}!", "{{Y|freeze grenade}}を1個作った！")]
    [TestCase("You tinker up three {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を3個作った！")]
    [TestCase("You tinker up four {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を4個作った！")]
    [TestCase("You tinker up five {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を5個作った！")]
    [TestCase("You tinker up six {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を6個作った！")]
    [TestCase("You tinker up seven {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を7個作った！")]
    [TestCase("You tinker up eight {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を8個作った！")]
    [TestCase("You tinker up nine {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を9個作った！")]
    [TestCase("You tinker up ten {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を10個作った！")]
    [TestCase("You tinker up 3 {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を3個作った！")]
    [TestCase("You tinker up 12 {{Y|freeze grenades}}!", "{{Y|freeze grenades}}を12個作った！")]
    public void PerformUITinkerBuild_TranslatesCountedSuccessPopups_WhenOwnerPatched(string source, string expected)
    {
        AssertPopupMessage(source, expected);
    ...
    [Test]
    public void PerformUITinkerBuild_TranslatesSuccessPopup_WithIndefiniteArticleAn_WhenOwnerPatched()
    {
        AssertPopupMessage(
            "You tinker up {{Y|an arc winder}}!",
            "{{Y|an arc winder}}を作った！");

        Assert.That(TinkeringBuildHitCount(), Is.EqualTo(1));
    }

    [Test]
    public void PerformUITinkerBuild_TranslatesSuccessPopup_WithPlainUntaggedItem_WhenOwnerPatched()
    {
        AssertPopupMessage(
            "You tinker up a laser pistol!",
            "a laser pistolを作った！");
    ...
    [Test]
    public void PerformUITinkerBuild_TranslatesMissingIngredient_WithSingleIngredient_WhenOwnerPatched()
    {
        DummyTinkeringBuildTarget.PopupMethod = nameof(DummyPopupShow.ShowFail);

        AssertPopupMessage(
            "You don't have the required ingredient: {{Y|copper wire}}!",
            "必要な材料が足りない: {{Y|copper wire}}！");
