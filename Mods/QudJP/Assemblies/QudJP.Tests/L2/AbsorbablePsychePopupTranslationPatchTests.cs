...
public sealed class AbsorbablePsychePopupTranslationPatchTests
{
    ...
    [Test]
    public void HandleEvent_TranslatesEncodePopup_WithColorTaggedEntityName_WhenOwnerPatched()
    {
        AssertPopupMessage(
            "You encode the psyche of {{R|Mind Flayer}} and gain +{{C|1}} {{Y|Ego}}!",
            "{{R|Mind Flayer}}の精神を刻みつけ、自我が+{{C|1}}上昇した！");

        Assert.That(AbsorbablePsycheHitCount(), Is.EqualTo(1));
    }

    [Test]
    public void HandleEvent_TranslatesRadiatePopup_WithColorTaggedEntityName_WhenOwnerPatched()
    {
        AssertPopupMessage(
            "You pause as the psyche of {{G|Psionic Urchin}} radiates into nothingness.",
            "{{G|Psionic Urchin}}の精神が無へと放射されていくのを見届ける。");

        Assert.That(AbsorbablePsycheHitCount(), Is.EqualTo(1));
    }

    [Test]
    public void HandleEvent_HitCountRemainsZero_WhenEmptyPopupEncountered_WhenOwnerPatched()
    {
        AssertPopupMessage(string.Empty, string.Empty);
    ...
    [Test]
    public void HandleEvent_TranslatesSequentialEncode_ThenRadiate_WhenOwnerPatched()
    {
        AssertPopupMessage(
            "You encode the psyche of Esper Hunter and gain +{{C|1}} {{Y|Ego}}!",
            "Esper Hunterの精神を刻みつけ、自我が+{{C|1}}上昇した！");

        var firstHitCount = AbsorbablePsycheHitCount();
        ...
        Assert.Multiple(() =>
        {
            Assert.That(firstHitCount, Is.EqualTo(1));
            Assert.That(AbsorbablePsycheHitCount(), Is.EqualTo(1));
        });
    }
