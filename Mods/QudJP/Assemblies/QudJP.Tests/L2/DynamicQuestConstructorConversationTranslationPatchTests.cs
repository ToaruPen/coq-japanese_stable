        ...
        WithPatchedConstructorMethods(() =>
        {
            ...
            Assert.Multiple(() =>
            {
                Assert.That(
                    ...
                    Does.Contain("かつて私の同胞は炉の世話に日々を費やしていた。塩の聖性を知ってから、われらは習わしを改め、新たな儀式を作った。残念ながら、やるべき用事がある。Mehmetがわれらの{{|遺物}}を失った。{{|錆の井戸}}の近くに{{|遺物}}があると知った。料理のために、それが必要だ。それを探し出し、われらのもとへ返してくれるか？それを取り戻してくれれば、あなたの助力には代価を支払う。{{|錆の井戸}}に運ばれたと聞いている。引き受けてくれるか？あなたの奉仕には報いる。"));
                Assert.That(
                    DummyDynamicQuestConstructorConversationTarget.Nodes,
                ...
                Assert.That(
                    DummyDynamicQuestConstructorConversationTarget.Nodes,
                    Does.Contain("{{|塩の祠}}にある{{|聖なる器}}のことを聞いたことはあるか、冒険者よ？それはわれらにとって神聖な祠だ。われらはしばしばそこへ巡礼し、忍耐について思索する。あなたも同じことをしてくれれば、われらの誉れとなる。引き受けてくれるか？あなたの奉仕には報いる。"));
                Assert.That(
                    DummyDynamicQuestConstructorConversationTarget.Nodes,
                    Does.Contain("*慎重にあたりを見回す*\n\n近くへ、友よ。私の{{|計画}}はもうすぐ成就する。残る手順はあと一つだ。{{|塩の祠}}へ行き、そこの{{|聖なる器}}に関わってくれる者が必要だ。いや、理由は話せない。引き受けてくれるか。あなたの奉仕には報いる。\n\n忍耐にかけて、このことは誰にも話すな。"));
                Assert.That(
                    DummyDynamicQuestConstructorConversationTarget.Nodes,
