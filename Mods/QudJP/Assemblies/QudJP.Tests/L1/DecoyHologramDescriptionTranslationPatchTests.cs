...
public sealed class DecoyHologramDescriptionTranslationPatchTests
{
    ...
    [Test]
    public void TranslateHologramDescription_PreservesNullShortProperty()
    {
        var description = new DummyDescriptionPart { Short = null };

        Assert.DoesNotThrow(() =>
            DecoyHologramDescriptionTranslationPatch.TranslateHologramDescriptionForTests(description));

        Assert.That(description.Short, Is.Null);
    }

    [Test]
    public void TranslateHologramDescription_TranslatedMarkerPrefixResultHasNoLeadingMarker()
    {
        var marker = new DummyDescriptionPart
        {
            Short = "\u0001Light stammers in parallax to form the image of an object. Suffix text.",
        };

        DecoyHologramDescriptionTranslationPatch.TranslateHologramDescriptionForTests(marker);

        Assert.That(marker.Short, Does.Not.StartWith("\u0001"));
    }

    [Test]
    public void TranslateHologramDescription_PreservesEmptyShortProperty()
    {
        var description = new DummyDescriptionPart { Short = string.Empty };

        DecoyHologramDescriptionTranslationPatch.TranslateHologramDescriptionForTests(description);

        Assert.That(description.Short, Is.EqualTo(string.Empty));
    }
