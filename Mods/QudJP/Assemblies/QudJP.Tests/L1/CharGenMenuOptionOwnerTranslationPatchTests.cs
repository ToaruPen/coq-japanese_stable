...
public sealed class CharGenMenuOptionOwnerTranslationPatchTests
{
    ...
    [Test]
    public void TranslateMenuOptionsForTests_EmptyListReturnsEmptyEnumerable()
    {
        var options = new List<DummyCharGenMenuOption>();

        var translated = CharGenMenuOptionOwnerTranslationPatch.TranslateMenuOptionsForTests(options).ToList();

        Assert.That(translated, Is.Empty);
    }

    [Test]
    public void TranslateChoiceTitlesForTests_EmptyListReturnsEmptyEnumerable()
    {
        var selections = new List<DummyChoiceWithColorIcon>();

        var translated = CharGenMenuOptionOwnerTranslationPatch.TranslateChoiceTitlesForTests(selections).ToList();

        Assert.That(translated, Is.Empty);
    }

    [Test]
    public void MutationsMenuBarPostfix_MixedKnownAndUnknownDescriptionsPartiallyTranslates()
    {
        WriteDictionary(("Choose Variant", "変種を選択"));
        var options = new List<DummyCharGenMenuOption>
        {
            new() { Description = "Unknown mutation option", InputCommand = "CmdUnknown" },
            new() { Description = "Choose Variant", InputCommand = "CmdChargenMutationVariant" },
        };
        ...
        Assert.Multiple(() =>
        {
            Assert.That(translated[0].Description, Is.EqualTo("Unknown mutation option"));
            Assert.That(translated[1].Description, Is.EqualTo("変種を選択"));
        });
    }

    [Test]
    public void GamemodeSelectionsPostfix_PreservesMarkerPrefixedTitle()
    {
        WriteDictionary(("Classic", "クラシック"), ("Explore Qud in the year 1000.", "西暦1000年のクドを探索する。"));
        var selections = new List<DummyChoiceWithColorIcon>
        {
            new()
            {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(translated[0].Title, Is.EqualTo("\u0001Classic"));
            Assert.That(translated[0].Description, Is.EqualTo("西暦1000年のクドを探索する。"));
        });
    }
