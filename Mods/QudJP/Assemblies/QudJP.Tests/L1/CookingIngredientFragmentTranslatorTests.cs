...
public sealed class CookingIngredientFragmentTranslatorTests
{
    ...
    [TestCase("{{C|1}} serving of {{w|山羊肉ジャーキー}}", "{{w|山羊肉ジャーキー}}{{C|1}}食分")]
    [TestCase("{{C|1}} serving of 凝縮した{{Y|サルヴ}}", "凝縮した{{Y|サルヴ}}{{C|1}}食分")]
    [TestCase("1 dram of {{w|honey}}", "{{w|ハチミツ}}1ドラム")]
    ...
    [TestCase("{{Y|starapple jam}}", "{{Y|スターアップルジャム}}")]
