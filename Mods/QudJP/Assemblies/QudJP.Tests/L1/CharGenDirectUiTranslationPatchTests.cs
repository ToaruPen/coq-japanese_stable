...
public sealed class CharGenDirectUiTranslationPatchTests
{
    ...
    [Test]
    public void AttributeUpdatedPostfix_TranslatesZeroPointCostTitle()
    {
        var target = new DummyCharGenAttributeSelectionControlTarget
        {
            data =
    ...
    [Test]
    public void AttributeUpdatedPostfix_TranslatesMultipleLineBonusSource()
    {
        WriteDictionary(("Calling", "職能"), ("Toughness", "頑健"));
        var target = new DummyCharGenAttributeSelectionControlTarget
        {
            data =
            {
        ...
        {
            Assert.That(target.tooltip.LastText, Is.EqualTo("職能による +2\n頑健による +1\n"));
            Assert.That(target.TitleButton.Title, Is.EqualTo("[3点]"));
        });
    }
