...
public sealed class ActiveEffectPopupQueueTranslatorTests
{
    ...
    [TestCase("the snapjaw is hobbled!", "snapjawは足を引きずっている！", "HobbledApply")]
    [TestCase("you are overwhelmed with terror!", "あなたは恐怖に圧倒された！", "TerrifiedApply")]
    [TestCase("the snapjaw begins healing.", "snapjawは回復を始めた。", "GeometricHealApply")]
    public void TryTranslateQueuedMessage_TranslatesAdditionalActiveEffectPatterns(
        string source,
        string expected,
        string expectedDetail)
    {
    ...
    [TestCase("")]
    [TestCase("unknown queue message")]
    ...
    public void TryTranslateQueuedMessage_PreservesWholeSourceColorWrapper()
    {
        ...
        {
            Assert.That(translated, Is.True);
            Assert.That(result, Is.EqualTo("{{y|あなたはもう毒を受けていない！}}"));
