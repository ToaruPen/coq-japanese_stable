...
public sealed class DescriptionDetailReturnTranslatorTests
{
    ...
    [Test]
    public void TryTranslate_CyberneticsChoiceDescriptionWithoutSlotPatternIsUntranslated()
    {
        WriteDictionary(("optical bioscanner", "光学バイオスキャナ"));
        ...
        {
            Assert.That(translated, Is.False);
            Assert.That(result, Is.EqualTo("optical bioscanner"));
            Assert.That(detail, Is.Empty);
        });
    }

    [Test]
    public void TryTranslate_CyberneticsChoiceLongDescriptionOnlyRulesBlockIsTranslated()
    {
        var source = "{{C|+2 Agility}}";
    ...
    [Test]
    public void TryTranslate_GameObjectUnitDescriptionLevelsPatternVariants()
    {
        WriteDictionary(("Temporal Fugue", "時間遁走"));

        Assert.Multiple(() =>
        {
            DescriptionDetailReturnTranslator.TryTranslate(
                "+1 levels",
                GameObjectUnitDescriptionKind,
                out var result1,
                out var detail1);
            Assert.That(result1, Is.EqualTo("レベル+1"));
            Assert.That(detail1, Is.EqualTo("GameObjectUnitDescription"));
            ...
            DescriptionDetailReturnTranslator.TryTranslate(
                ...
                out var result10,
                out var detail10);
            Assert.That(result10, Is.EqualTo("レベル+10"));
            Assert.That(detail10, Is.EqualTo("GameObjectUnitDescription"));
        });
    }
