...
public sealed class DescriptionDetailReturnTranslatorTests
{
    ...
    [Test]
    public void TryTranslate_StripsMarkerForTinkerDataDescriptionKind()
    {
        var source = "\u0001\n{{rules|Makes a batch of two.}}\n\nBody text here.\n";
    ...
    [Test]
    public void TryTranslate_StripsMarkerForGameObjectUnitDescriptionKind()
    {
        WriteDictionary(("Tinkering", "工匠"));

        var source = "\u0001Has every Tinkering skill";
        ...
        {
            Assert.That(translated, Is.True);
            Assert.That(result, Is.EqualTo("Has every Tinkering skill"));
            Assert.That(detail, Is.Empty);
        });
    }

    [Test]
    public void TryTranslate_ReturnsUnknownKindAsUnsupported()
    {
        var translated = DescriptionDetailReturnTranslator.TryTranslate(
            "some text",
            DescriptionDetailReturnKind.Unknown,
            out var result,
            out var detail);
