...
public sealed class CookingRecipeDisplayNameTranslatorTests
{
    ...
    [TestCase("{{W|カムシュルウールの Sauteed Meat with Open Flame and Savory Char}}", "{{W|カムシュルウールのソテー肉：直火と旨味炭入り}}")]
    [TestCase("{{W|カムシュルウールの Dried Balm Inside of Roast 山羊肉ジャーキー and Honey}}", "{{W|カムシュルウールの乾いた軟膏：ロースト山羊肉ジャーキーとハチミツ入り}}")]
    [TestCase("{{W|カムシュルウールの Yogurt with 甲虫ジャーキー, Meat Rice, and Meat Kugel}}", "{{W|カムシュルウールのヨーグルト：甲虫ジャーキー、肉飯、肉クーゲル入り}}")]
    [TestCase("{{W|カムシュルウールの Meat over Honey Bread、薬、and Stew}}", "{{W|カムシュルウールの肉：ハチミツパン、薬、シチューがけ}}")]
    [TestCase("{{W|カムシュルウールの Matz on Top of Fermented Yondercane, Tossed Mazebeard Gland Paste, and Flaky Petals}}", "{{W|カムシュルウールのマッツァ：発酵ヨンダーケーン、和え迷髭腺ペースト、フレーク状花びらのせ}}")]
    [TestCase("{{W|カムシュルウールの Hoarshroom-Ice Mashed Elder Tartbeard}}", "{{W|カムシュルウールのホアシュルーム氷すり潰し年老いた酸髭}}")]
