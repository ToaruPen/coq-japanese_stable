...
- 最厳格Linter設定: Python は Ruff `select = ["ALL"]` + McCabe C ≤ 10 + complexity ≤ 10, C# は `TreatWarningsAsErrors` + Nullable enable + `EnforceCodeStyleInBuild` + `EnableNETAnalyzers` + `AnalysisLevel=latest-all` + SonarAnalyzer.CSharp（マジックナンバー S109、エラー握りつぶし S2737/S2221、ハードコード資格情報 S2068/S6418 を検出）
...
Wave 8 (After Wave 7 — polish, 3 parallel):
...
└── Task 20: GitHubリリース準備 (depends: 18, 19, 21) [quick]
...
| 7 | 1 | T17 → ultrabrain |
| 8 | 3 | T18 → unspecified-high, T19 → writing, T21 → unspecified-high |
| 9 | 1 | T20 → quick |
