...
| covered_by_owner_route | `XRL.World.Parts/Campfire.cs::Campfire.CookFromRecipe` | 23 | `covered_by_owner_route` for the HSE cooking surfaces that flow through the existing `spice.cooking.ate[0]` popup pattern and the same recipe display/meal description owner routes used by sibling campfire paths |
...
| covered_by_owner_route | `XRL.World.Parts/Campfire.cs::Campfire.CookPresetMeal` | 5 | `covered_by_owner_route` for the `spice.cooking.ate[0]` HSE surface through the existing anchored popup pattern |
