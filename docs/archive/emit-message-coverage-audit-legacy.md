...
- **Guaranteed by repository tests:** the repo dictionary now translates tested player and incoming hit families, tested player and incoming weapon-miss-with-roll families, a representative armor-penetration failure, cannot-reach, pass-through, bleeding stop / exact nosebleed / hemorrhage, HP lose/recover, singular harvest, self-bleeding, freezing-effect damage, and `acid!`-terminated acid families through the emit route.[^l1-repo][^l2-hit-acid]
...
| Player weapon miss-with-roll | `You miss with your レンチ! [10 vs 10]` | `Messaging.EmitMessage` combat route | Repository pattern proven through emit route | L1 ordering-sensitive repo-dict translator coverage already existed, and the expanded L2 emit-route regression now proves the representative production-dictionary family end-to-end through the emit owner path.[^l1-repo][^l1-ordering][^l2-hit-acid] | emit-pattern-dict | **covered** |
...
| Incoming weapon miss-with-roll | `The ウォーターヴァイン農家 misses you with his 鉄の蔓刈り斧! [3 vs 7]` | `Messaging.EmitMessage` combat route | Repository pattern proven through emit route | The repo already had L1 coverage for the stable incoming weapon-miss skeleton, and the expanded L2 emit-route regression now proves a representative production-dictionary family through the producer-owned emit seam.[^l1-repo][^l2-hit-acid] | emit-pattern-dict | **covered** |
...
| Freezing-effect damage | `You take 14 damage from 監視官イラメの freezing effect!` | emit-message status/effect route | Tested representative shape is proven through emit route | The repo already had L1 repo-dict coverage for the stable freezing-effect damage skeleton, and the expanded L2 emit-route regression now proves a representative production-dictionary message end-to-end on the producer-owned emit seam.[^l1-repo][^l2-hit-acid] | emit-pattern-dict | **covered** |
...
1. **Projectile combat is still under-proven.** It dominates the producer surface; stable hit/miss representatives are now route-tested, but broader crit, suppressive-fire, and related variants still are not.
...
1. Add focused L2 emit-route tests for the remaining high-volume projectile families, especially crit variants, suppressive-fire, and “shot goes wild”.
2. Expand status/effect proof from the newly tested representative shapes to the remaining bleed-start / another-wound / heavier-or-lighter nosebleed variants.
...
Keep the result synchronized by rerunning the command above after each emit-coverage batch update.
...
[^l2-emit]: `Mods/QudJP/Assemblies/QudJP.Tests/L2/CombatAndLogMessageQueuePatchTests.cs:505-715`
[^l2-mixed]: `Mods/QudJP/Assemblies/QudJP.Tests/L2/CombatAndLogMessageQueuePatchTests.cs:572-607`
[^l2-hit-acid]: `Mods/QudJP/Assemblies/QudJP.Tests/L2/CombatAndLogMessageQueuePatchTests.cs:609-759`
